 ...
 *
 * $Id: valarray.cc,v 1.15 1996/09/26 01:12:57 philippe Exp $
 *
 ***************************************************************************
 *
...
// operator[] for slice
   ...
   {
      valarray<T> tmp_array(sl.length());
...
// copy ctor and assignment for slice
   ...
   {
	  valarray<T> tmp_array(sl_ar.get_slice().length());
      
      size_t ind = sl_ar.get_slice().start();
      size_t cpt = 0; 

      while( cpt < sl_ar.get_slice().length() )
	  {
        tmp_array[cpt] = (*(sl_ar.get_ref_mem_array()))[ind];
		ind+= sl_ar.get_slice().stride();
		cpt++;
      }
   memory_array._replace(tmp_array._RW_get_memory_array()._RW_get_storage(),sl_ar.get_slice().length());
