 ...
 *
 * $Id: algorithm.cc,v 1.4 1996/08/30 00:35:17 smithey Exp $
 *
 ***************************************************************************
 *
 * Copyright (c) 1994
                             ...
                             Distance buffer_size, T*, Compare _RWSTD_COMP)
{
    ...
    {
      __merge_sort_with_buffer(first, middle, buffer, _RWSTD_STATIC_CAST(Distance*,0),
			       _RWSTD_STATIC_CAST(T*,0), _RWSTD_COMP);
      __merge_sort_with_buffer(middle, last, buffer, _RWSTD_STATIC_CAST(Distance*,0),
			       _RWSTD_STATIC_CAST(T*,0), _RWSTD_COMP);
    }
    __merge_adaptive(first, middle, last, Distance(middle - first), 
                     ...
                     RandomAccessIterator last, T*, Compare _RWSTD_COMP)
{
    ...
    for (RandomAccessIterator i = middle; i < last; ++i)
        if (_RWSTD_COMP(*i,*first)) 
            __pop_heap(first, middle, i, T(*i), _RWSTD_COMP, distance_type(first));
