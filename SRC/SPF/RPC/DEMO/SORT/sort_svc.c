...
main(int argc)
{
	...
	if (argc > 1) {
		fprintf(stderr,"Syntax:   sortd\n");
		fprintf(stderr,"Function: sortd is the OS-9/OS9000 server for remote sort \n");
	...
	if (!svc_register(transp, SORTPROG, SORTVERS, sortprog_1, IPPROTO_UDP)) {
		fprintf(stderr, "unable to register (SORTPROG, SORTVERS, udp).\n");
		exit(1);
	}
	...
	if (!svc_register(transp, SORTPROG, SORTVERS, sortprog_1, IPPROTO_TCP)) {
		fprintf(stderr, "unable to register (SORTPROG, SORTVERS, tcp).\n");
		exit(1);
	}
	svc_run();
