...
# High-Level Driver for Archive Viper Drive
#
...
OBJ			= sbteac					# the high-level driver name
...
OSDEFS		= $(SYSDEFS)/sbf.h $(SYSDEFS)/types.h $(SYSDEFS)/MACHINE/reg.h \
			  $(SYSDEFS)/procid.h $(SYSDEFS)/path.h $(SYSDEFS)/modes.h \
			  $(SYSDEFS)/sg_codes.h $(CDEFS)/errno.h

DEFS		= ./$(OBJ).h $(COMDEFS) $(OSDEFS)
...
CFLAGS		= -qst=$(TMPDIR) -O=2 $(SRCHDIRS) $(DEBUG)
