...
# 02 97/12/03 Included 68020 unix.l instead of 68000.  MHQco02973     ljs   #
# 03 98/01/24 MHQco03317 - Removed -as=-bt from APARMS macro.               #
...
# 04 98/07/15 Added MWOS_LDIR to allow MWOS_IDIR and MWOW_RDIR to       mgh #
...
# 05 98/10/29 added OSDIR, COPTs for module owner, attributes, and      mgh #
#             moved includes for baserule.tpl and trgtrule.tpl              #
#             ---- Makefile Support Sub-component v1.3   Released ----      #  
# 06 99/07/06 Added CPUDIR MACRO                                        mgh #
# 07 99/07/07 Removed superfluous -i option from MWOS_CSLLIBS (CF2653)  ajk #
# 08 00/01/24 Moved ISP macros to isp.tpl                               rds #
# 09 00/08/26 Change unix.l to use BASLIB to match other trgt templates mgh #
#             $$                    <RELEASE_INFO>                     $$   #
#############################################################################
#	make template file for all hosts OS-9/68020 target 						#
#############################################################################
...
MWOS_LDIR       =   k020                # Lower case directory name (Linkers)
MWOS_IDIR		=	RELS/k68k
MWOS_RDIR		=	RELS/k020
MWOS_OSDIR		=	$(MWOS)/OS9
MWOS_ODIR		=	$(MWOS_OSDIR)/68020/CMDS

MWOS_DDIR		=	$(MWOS)/SRC/DEFS
MWOS_CDEF		=	$(TOOLSET)/SRC/DEFS

MWOS_DFTDEF		=	$(MWOS_OSDIR)/68020/DEFS
...
MWOS_BASLIB		=	$(MWOS_OSDIR)/68020/LIB		# First Fallback to find LIB
MWOS_BASCLIB	=	$(TOOLSET)/OS9/68020/LIB

MWOS_LIBDIR		=	$(MWOS_OSDIR)/68020/LIB		# Processor Specific LIBS
MWOS_CLIBDIR	=	$(TOOLSET)/OS9/68020/LIB
...
# Lint Options
LINTPARMS		=	-d_UCC -dANSI_EXT -d_OSK -d_MPF68020 -d_FPF881 -d_BIG_END
